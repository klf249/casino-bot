  ...
  async execute({ client, message, args, embed, status }) {
    ...
    if (!Number.isInteger(itemId) || itemId <= 0 || !field || !rawValue) {
      return message.reply({
        embeds: [embed({ variant: 'error', description: status(false, 'Usage: +drawedit {id} {champ} {valeur}') })],
      })
    }
    ...
    if (field === 'name') patch.name = rawValue
    ...
    else if (field === 'weight') patch.weight = Number.parseFloat(rawValue)
      ...
      return message.reply({
        embeds: [embed({ variant: 'error', description: status(false, 'Champs autorisés: name, category, weight, rewardType, rewardValue, roleId, emoji, enabled, sort') })],
      })
    }
    ...
    const updated = client.store.updateCasinoDrawItem(message.guild.id, itemId, patch)
    if (!updated.ok) {
      return message.reply({
      ...
      embeds: [
        embed({
          ...
          description: [
            ...
            formatDrawItemAdminLine(updated.item),
          ].join('\n'),
        }),
      ],
    })
